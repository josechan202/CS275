...
def application(environ, start_response):
    ...
    if environ["REQUEST_METHOD"]== 'GET':
        ...
        sql = "SELECT Club.club_id, Club.clubname FROM Club, Admins WHERE Admins.username = %s AND Admins.club_id = Club.club_id AND Club.clubname LIKE %s LIMIT %s, %s"
        rows_count = cursor.execute(sql, (username, "%" + query + "%", int(startIndex), int(groupSize)))
        if rows_count == 0:
            pass
